...
export async function GET() {
  const agreements = await prisma.companyAgreement.findMany({
    orderBy: {
      contractStartDate: "desc",
    },
  });

  return NextResponse.json(agreements.map(serializeAgreement));
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const agreement = await prisma.companyAgreement.create({
      data,
    });
    ...
    return NextResponse.json(
      { message: "Failed to save company agreement." },
      { status: 500 }
    );
  }
}
